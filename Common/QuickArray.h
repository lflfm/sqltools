...
namespace Common
{
    ...
    QuickArray<T>::QuickArray<T> (size_t capacity,  size_t growBy)
    {
        FakeArray::castPtr(this)->construct(sizeof(T), capacity, growBy);
    }

    template <class T> inline
    QuickArray<T>::QuickArray<T> (const QuickArray<T>& array)
    {
    ...
    void QuickArray<T>::erase (size_t index, size_t count)
    {
        ...
        for (size_t i = index, end = index + count; i < end && i < m_size; i++)
            operator[](i).~T();
