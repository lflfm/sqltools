...
#define LOGWARN(logObj, fmt, ...) logObj.Log(framework::Diagnostics::LogLevel::Warn, __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__);
...
#define LOGWARNP(logObjPtr, fmt, ...) logObjPtr->Log(framework::Diagnostics::LogLevel::Warn, __FILE__, __LINE__, __FUNCTION__, fmt, ##__VA_ARGS__);
	...
	namespace Diagnostics
	{
		...
		private:
			struct StreamInfo
			{
				...
				StreamInfo(ostream* pStream, bool owned, LogLevel level)
				{
		...
		public:
			CLogger(framework::Diagnostics::LogLevel level, LPCTSTR name, 
				...
				static_cast<int>(LogItem::LoggerName) | static_cast<int>(LogItem::LogLevelI)) 
			...
			void AddOutputStream(ostream& os, bool own, LogLevel level)
			{
				AddOutputStream(&os, own, level);
			}

			void AddOutputStream(ostream& os, bool own)
			{
			...
			void Log(LogLevel level, LPCTSTR file, INT line, LPCTSTR func, LPCTSTR fmt, ...)
			{
				...
				for(vector<StreamInfo>::iterator iter = m_outputStreams.begin(); iter < m_outputStreams.end(); iter++)
				{
					...
					if(m_loggableItem & static_cast<int>(LogItem::LogLevelI))
					{
						TCHAR strLevel[4];
		...
		private:			
			...
			template <class T> inline bool write(T data, bool written, ostream* strm)
			{
				if(written == true)
				{
					(*strm) << _T(" ");
			...
			inline bool write_datetime(bool written, ostream* strm)
			{
				if(written == true)
				{
					(*strm) << _T(" ");
