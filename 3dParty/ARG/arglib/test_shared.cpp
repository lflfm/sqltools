...
//#define ARG_TEST_FOR_CAST_COMPILE_ERROR
...
namespace example4
{
	...
	{
	public:
		virtual void set_spouse(uncounted_ptr<person> value) = 0;
	...
	{
	public:
		virtual void set_spouse(uncounted_ptr<person> value);
		virtual uncounted_ptr<person> get_spouse() const;
	private:
		uncounted_ptr<person> husband;
	};
	...
	public:
		...
		virtual uncounted_ptr<person> get_spouse() const;
	private:
		uncounted_ptr<person> wife;
	};


	void woman::set_spouse(uncounted_ptr<person> value) { husband = value; }
	void man::set_spouse(uncounted_ptr<person> value)    { wife = value; }
	uncounted_ptr<person> woman::get_spouse() const   { return husband; }
	uncounted_ptr<person> man::get_spouse() const     { return wife; }

	void marry(counted_ptr<person> a, counted_ptr<person> p)
	{
		if (a.get() && p.get())
		{
			a->set_spouse(p); p->set_spouse(a);
		}
	}
	...
	void f()
	{
		...
		uncounted_ptr<person> pippa;
		uncounted_ptr<person>   alan; 
		...
		{
			counted_ptr<person> p(new woman);
			counted_ptr<person>   a(new man);
...
#ifdef ARG_TEST_FOR_CAST_COMPILE_ERROR
	{
