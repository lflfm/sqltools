...
void CSizingControlBarG::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    ...
    CPoint ptOrgBtn;
    if (bHorz)
        ptOrgBtn = CPoint(rc.left - 13-3, rc.top+3); // (ak)
    else
        ptOrgBtn = CPoint(rc.right - 12-3, rc.top - 13-3); // (ak)
...
void CSCBButton::Paint(CDC* pDC)
{
    ...
    pDC->SetTextColor(::GetSysColor(bActive ? COLOR_CAPTIONTEXT : COLOR_BTNTEXT)); // (ak)
    int nPrevBkMode = pDC->SetBkMode(TRANSPARENT);
    CFont font;
    int ppi = pDC->GetDeviceCaps(LOGPIXELSX);
    int pointsize = MulDiv(70, 96, ppi); // 6 points at 96 ppi // (ak)
    font.CreatePointFont(pointsize, _T("Marlett"));
    CFont* oldfont = pDC->SelectObject(&font);

    pDC->TextOut(ptOrg.x + 2, ptOrg.y + 2+1, CString(_T("r"))); // x-like // (ak)
