...
/////////////////////////////////////////////////////////////////////////
//
// 02.06.2002 (ak) flicker reduction but the result is not so good
    ...
    for (int i = nFirst; i <= nLast; i++)
    {
        ...
        if (bIsSizingBar)
        {
            nLengthActual += bHorz ? pBar->m_szHorz.cx - 2 : 
                pBar->m_szVert.cy - 2;
            nWidthMax = max<int>(nWidthMax, bHorz ? pBar->m_szHorz.cy :
                pBar->m_szVert.cx);
        }
        else
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    m_szHorz.cx = max<int>(m_szMinHorz.cx, pApp->GetProfileInt(
        szSection, _T("sizeHorzCX"), m_szHorz.cx));
    m_szHorz.cy = max<int>(m_szMinHorz.cy, pApp->GetProfileInt(
        szSection, _T("sizeHorzCY"), m_szHorz.cy));

    m_szVert.cx = max<int>(m_szMinVert.cx, pApp->GetProfileInt(
        szSection, _T("sizeVertCX"), m_szVert.cx));
    m_szVert.cy = max<int>(m_szMinVert.cy, pApp->GetProfileInt(
        szSection, _T("sizeVertCY"), m_szVert.cy));

    m_szFloat.cx = max<int>(m_szMinFloat.cx, pApp->GetProfileInt(
        szSection, _T("sizeFloatCX"), m_szFloat.cx));
    m_szFloat.cy = max<int>(m_szMinFloat.cy, pApp->GetProfileInt(
...
void CSCBDockContext::StartDrag(CPoint pt)
{
    ...
    if (m_pBar->IsFloating())
        m_bForceFrame = TRUE; // (ak) disable docking !!!
    Track();
}
